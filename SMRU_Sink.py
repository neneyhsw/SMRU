...
N = 20  # quantities of Sensor

# the coordinate of Data sink node
# [x, y, id, power in needed, be covered number]
Data_sink_node = [[25, 25, 1, 0, 0], [25, 75, 2, 0, 0], [75, 25, 3, 0, 0], [75, 75, 4, 0, 0]]
...
# [x, y, flag(whether drop out the loop), the number of covering sensors, id, the id of covered sensor, sector angle]
...
sum_Data_Sink_Require_Power1 = float(0)
sum_Data_Sink_Require_Power2 = float(0)
sum_Data_Sink_Require_Power3 = float(0)
sum_Data_Sink_Require_Power4 = float(0)
sum_Give_Power1 = 0
sum_Give_Power2 = 0
sum_Give_Power3 = 0
sum_Give_Power4 = 0
...
while(True):
    ...
    for i in range(N):
        while(1):
            ...
            for j in range(4):
                Distance_between_Sensor_Sink = Distance_between_two_points(Data_sink_node[j][0], Data_sink_node[j][1], Charger_Point[i][0], Charger_Point[i][1])
                if (Distance_between_Sensor_Sink <= sink_r):
                    DbSS_Flag = True
                    break
            if (DbSS_Flag == True):
                DbSS_Flag = False
    ...
    # reset Data sink node
    for i in range(sink_num):
        Data_sink_node[i][3] = 0
        Data_sink_node[i][4] = 0
    ...
    while(True):
        ...
        #### Sink Area ####
        print("Data Sink Node =", Data_sink_node)
        # sink rate calculate
        for i in range(N):
            min_r = 200
            Data_Sink_Id = 0
            for j in range(sink_num):
                Sensor_r, Sensor_theta = Convert_to_Polor(
                    Charger_Point[i][0], Charger_Point[i][1], Data_sink_node[j][0], Data_sink_node[j][1])
                Sensor_theta, Trash_Can_start_arm, Trash_Can_end_arm = Theta_Conversion(
                    Sensor_theta, (angle/2))
                
                if (Sensor_r < min_r):
                    Data_Sink_Id = Data_sink_node[j][2]
                    min_r = Sensor_r
            for k in range(sink_num):
                if (Data_sink_node[k][2] == Data_Sink_Id):
                    Data_sink_node[k][3] += sink_rate



        # check charger point that around sink point whether can cover the sink
        # if cover point > 1, then search 360, else, change angle to sink point
        for i in range(Charger_Num):
            for j in range(sink_num):
                Sensor_r, Sensor_theta = Convert_to_Polor(
                    Truly_Charger_Point[i][0], Truly_Charger_Point[i][1], Data_sink_node[j][0], Data_sink_node[j][1])
                Sensor_theta, start_arm, end_arm = Theta_Conversion(
                    Sensor_theta, (angle/2))
                if (Sensor_r <= cr):
                    '''
                    # exam the sink whether is covered
                    Max_count = 0
                    temp_Sensor_theta, start_arm, end_arm = Theta_Conversion(
                        Truly_Charger_Point[i][6], (angle/2))
                    for k in range(N):
                        temp_Sensor_r, Trash_temp_Sensor_theta = Convert_to_Polor(
                            Truly_Charger_Point[i][0], Truly_Charger_Point[i][1], Charger_Point[k][0],
                            Charger_Point[k][1])
                        if (temp_Sensor_r <= cr):
                            if (temp_Sensor_theta == 0 and temp_Sensor_r == 0):
                                Max_count += 1
                    ...
                    for k in range(sink_num):
                        temp_Sensor_r, temp_Sensor_theta = Convert_to_Polor(
                            Truly_Charger_Point[i][0], Truly_Charger_Point[i][1], Data_sink_node[k][0],
                            Data_sink_node[k][1])
                        if (temp_Sensor_r <= cr):
                            if (temp_Sensor_theta == 0 and temp_Sensor_r == 0):
                                Max_count += 1
                                ...
                                if (start_arm <= temp_Sensor_theta and end_arm >= temp_Sensor_theta):
                                    Max_count += 1
                    if (Max_count > Truly_Charger_Point[i][3]):
                        Data_sink_node[j][4] += 1
                        print("original covered ", Data_sink_node[j], "Truly =", Truly_Charger_Point[i])
                        continue  # if be covered, then skip the charger
                        #break
                    '''


                    if (Truly_Charger_Point[i][3] > 1):
                        temp_Previous_Max_Sensor_Number = 0
                        # examine the uncovered point whether can cover more point
                        for k in range(360):
                            Max_count = 0
                            ...
                            for m in range(N):
                                temp_Sensor_r, temp_Sensor_theta = Convert_to_Polor(
                                    Truly_Charger_Point[i][0], Truly_Charger_Point[i][1], Charger_Point[m][0],
                                    Charger_Point[m][1])
                                temp_Sensor_theta, Trash_Can_start_arm, Trash_Can_end_arm = Theta_Conversion(
                                    temp_Sensor_theta, (angle/2))
                                if (temp_Sensor_r <= cr):
                                    if (temp_Sensor_theta == 0 and temp_Sensor_r == 0):
                                        ...
                                        if (start_arm <= temp_Sensor_theta and end_arm >= temp_Sensor_theta):
                                            Max_count += 1
                            for m in range(sink_num):
                                temp_Sensor_r, temp_Sensor_theta = Convert_to_Polor(
                                    Truly_Charger_Point[i][0], Truly_Charger_Point[i][1], Data_sink_node[m][0],
                                    Data_sink_node[m][1])
                                temp_Sensor_theta, Trash_Can_start_arm, Trash_Can_end_arm = Theta_Conversion(
                                    temp_Sensor_theta, (angle/2))
                                if (temp_Sensor_r <= cr):
                            ...
                            if (Max_count > temp_Previous_Max_Sensor_Number):
                                ...
                                temp_Best_start_arm = start_arm
                                temp_Best_end_arm = end_arm
                        if (temp_Previous_Max_Sensor_Number > Truly_Charger_Point[i][3]):
                            Truly_Charger_Point[i][6] = temp_Max_Sensor_Degree
                            Data_sink_node[j][4] += 1
                            #print("rotate to cover ", Data_sink_node[j], "Truly =", Truly_Charger_Point[i])
                    else:
                        Truly_Charger_Point[i][6] = Sensor_theta
                        Data_sink_node[j][4] += 1
                        #print("1 and to cover ", Data_sink_node[j], "Truly =", Truly_Charger_Point[i])

        # fill the require of power to be enough
        for i in range(sink_num):
            while (True):
                if (Data_sink_node[i][3] <= Data_sink_node[i][4]):
                    break
                if (Data_sink_node[i][3] > Data_sink_node[i][4]):
                    randnum1 = np.random.randint(low=-10, high=(11), size=None)
                    randnum2 = np.random.randint(low=-10, high=(11), size=None)
                    temp_Array = np.array([Data_sink_node[i][0]+randnum1, Data_sink_node[i][1]+randnum2, -1, -1, -1, -1, -1])

                    Sensor_r, Sensor_theta = Convert_to_Polor(
                        temp_Array[0], temp_Array[1], Data_sink_node[i][0], Data_sink_node[i][1])
                    Sensor_theta, Trash_Can_start_arm, Trash_Can_end_arm = Theta_Conversion(
                        Sensor_theta, (angle/2))
                    temp_Array[6] = Sensor_theta

                    Truly_Charger_Point = np.row_stack(
                        (Truly_Charger_Point, temp_Array))
                    Data_sink_node[i][4] += 1
                    Charger_Num += 1
                    Truly_Row += 1

        # calculate sink require
        print("Data Sink Node =", Data_sink_node)
        sum_Data_Sink_Require_Power1 += Data_sink_node[0][3]
        sum_Data_Sink_Require_Power2 += Data_sink_node[1][3]
        sum_Data_Sink_Require_Power3 += Data_sink_node[2][3]
        sum_Data_Sink_Require_Power4 += Data_sink_node[3][3]

        sum_Give_Power1 += Data_sink_node[0][4]
        sum_Give_Power2 += Data_sink_node[1][4]
        sum_Give_Power3 += Data_sink_node[2][4]
        sum_Give_Power4 += Data_sink_node[3][4]


        '''
        Sink_Row = 0
        for i in range(sink_num):
            for j in range(math.ceil(Data_sink_node[i][3])):  # round up
                randnum1 = np.random.randint(low=-10, high=(11), size=None)
                randnum2 = np.random.randint(low=-10, high=(11), size=None)
                temp_Array = np.array([Data_sink_node[i][0]+randnum1, Data_sink_node[i][1]+randnum2, -1])

                Sensor_r, Sensor_theta = Convert_to_Polor(
                    temp_Array[0], temp_Array[1], Data_sink_node[i][0], Data_sink_node[i][1])
                Sensor_theta, Trash_Can_start_arm, Trash_Can_end_arm = Theta_Conversion(
                    Sensor_theta, (angle/2))
                temp_Array[2] = Sensor_theta

                if (i == 0 and j == 0):
                    Sink_Charger_Point = temp_Array.copy()
                else:
                    Sink_Charger_Point = np.row_stack(
                        (Sink_Charger_Point, temp_Array))
                    Sink_Row += 1
        '''
...
avg_Data_Sink_Require_Power1 = (sum_Data_Sink_Require_Power1 / (while_count-1))
avg_Data_Sink_Require_Power2 = (sum_Data_Sink_Require_Power2 / (while_count-1))
avg_Data_Sink_Require_Power3 = (sum_Data_Sink_Require_Power3 / (while_count-1))
avg_Data_Sink_Require_Power4 = (sum_Data_Sink_Require_Power4 / (while_count-1))
avg_Give_Power1 = (sum_Give_Power1 / (while_count-1))
avg_Give_Power2 = (sum_Give_Power2 / (while_count-1))
avg_Give_Power3 = (sum_Give_Power3 / (while_count-1))
avg_Give_Power4 = (sum_Give_Power4 / (while_count-1))

print("sum_time =", sum_time, "sec")
print("avg_sum_time =", avg_sum_time, "sec")
print("avg_Charger_Num =", avg_Charger_Num, "\n\n")
#print("sum_Data_Sink_Require_Power1 =", sum_Data_Sink_Require_Power1, "units")
#print("sum_Data_Sink_Require_Power2 =", sum_Data_Sink_Require_Power2, "units")
#print("sum_Data_Sink_Require_Power3 =", sum_Data_Sink_Require_Power3, "units")
#print("sum_Data_Sink_Require_Power4 =", sum_Data_Sink_Require_Power4, "units")
print("avg_Data_Sink_Require_Power1 =", avg_Data_Sink_Require_Power1, "units")
print("avg_Data_Sink_Require_Power2 =", avg_Data_Sink_Require_Power2, "units")
print("avg_Data_Sink_Require_Power3 =", avg_Data_Sink_Require_Power3, "units")
print("avg_Data_Sink_Require_Power4 =", avg_Data_Sink_Require_Power4, "units\n")
#print("sum_Give_Power1 =", sum_Give_Power1, "units")
#print("sum_Give_Power2 =", sum_Give_Power2, "units")
#print("sum_Give_Power3 =", sum_Give_Power3, "units")
#print("sum_Give_Power4 =", sum_Give_Power4, "units\n\n")
print("avg_Give_Power1 =", avg_Give_Power1, "units")
print("avg_Give_Power2 =", avg_Give_Power2, "units")
print("avg_Give_Power3 =", avg_Give_Power3, "units")
print("avg_Give_Power4 =", avg_Give_Power4, "units\n\n")
...
'''
# sink Charger
for i in range(Sink_Row+1):
    Previous_Max_Sensor_Number = 0

    Sink_Charger_Point[i][2], Best_start_arm, Best_end_arm = Theta_Conversion(
        Sink_Charger_Point[i][2], (angle/2))

    # set sector range of drawing
    if (i == 0):
        wedge = mpatches.Wedge(
            (Sink_Charger_Point[i][0], Sink_Charger_Point[i][1]), cr, Best_start_arm, Best_end_arm, ec="none")
    else:
        wedge = mpatches.Wedge(
            (Sink_Charger_Point[i][0], Sink_Charger_Point[i][1]), cr, Best_start_arm, Best_end_arm, ec="none")

    # set parameters of sector and draw
    wedge.set_label("Sink_Charger")
    wedge.set_edgecolor("black")
    wedge.set_facecolor("LightPink")
    wedge.set_alpha(0.1)
    plt.gcf().gca().add_artist(wedge)

    if (i == 0):
        plt.scatter(Sink_Charger_Point[i][0], Sink_Charger_Point[i][1], alpha=0.8,
                    color="LightPink", marker="o", label="Sink Charger")  # set the center of sector
    else:
        plt.scatter(Sink_Charger_Point[i][0], Sink_Charger_Point[i][1],
                    alpha=0.8, color="LightPink", marker="o")  # set the center of sector
'''

'''
for i in range(sink_num):
    circle = plt.Circle((Data_sink_node[i][0], Data_sink_node[i][1]), sink_r, edgecolor="black",
                        facecolor="Orange", fill=True, alpha=0.08)
